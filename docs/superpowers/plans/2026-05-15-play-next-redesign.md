...
> **Execution note (2026-05-15) — Step-3 code below was partly fictional; shipped real-grounded in commit `0b1662c`.** The plan's loose `{ok:boolean;reason?}` return + WHERE-filter ownership + "set dismissed_at INSTEAD of dismissed" were wrong vs the real `dismissRec`. Reality (preserved): all 3 actions return the precise union `{ ok: true } | { ok: false; reason: "unauthorized" | "not-found" }` and use the explicit ownership SELECT→not-found/unauthorized→update pattern (the file's established idiom, shared with `saveRecForLater`/`playRec`). `dismissRec` was already a soft update (`dismissed:true`, deliberately never deletes — feeds the cache-hit window + rerank negative-context); T13 makes it set **BOTH** `dismissed:true` AND `dismissedAt:new Date()` (NOT "instead of") so T5's `softNegativePenalty` (read by T12's getRecs negMap) gets a timestamp. `snoozeRec(recId)` = fixed 30-day (`snoozedUntil`, no `dismissed`); `neverAgainRec(recId)` = `{neverAgain:true, dismissedAt:now}`. `saveRecForLater`/`playRec` untouched. 9 unit tests (owned/not-found/unauthorized × 3, exact `.set()` assertions); full suite 859/859.
> - **Parking-lot carry-forward (code-review finding, NOT a T13 defect — out of the plan's 3-action mandate):** `saveRecForLater` + `playRec` still `.set({ dismissed: true })` WITHOUT `dismissedAt`, so post-T13 an explicit dismiss feeds the soft-neg decay but a "save for later" / "now playing" does not. This is a genuine product question — arguably correct-by-intent (interest ≠ rejection). Decide explicitly at branch-finish: either stamp `dismissedAt` there too for parity, or add a one-line "intentional: interest ≠ rejection" comment at those `.set()` calls. Also optional post-plan cleanup: extract a shared owned-rec mutation helper across all 5 actions (deferred — refactoring mid-plan is scope creep).
...
> **Execution note (2026-05-15) — shipped real-grounded in commits `3308da8` + `8b9d21d`.** Two plan assumptions were wrong for this repo: (1) the plan's RTL test (`@testing-library/react` + `toBeInTheDocument`) — that dep is NOT installed and vitest runs `environment:"node"` (no DOM); repo convention (per `tests/unit/recaps/featured-list-card.test.tsx`) is `renderToStaticMarkup` + string assertions, which the shipped 10-test file uses. (2) The plan's Tailwind tokens (`brand-purple`/`muted`/`muted-foreground`) don't exist — Tailwind v4, theme in `app/globals.css @theme`; the sibling `components/recs/rec-card.tsx` uses CSS-var arbitrary-values, so the atoms use the REAL tokens: `--accent`(#7c5cff)/`--accent-fg`/`--text-dim`/`--bg-elev` and `--pixel`(#ffb84a amber, wildcard = loudest + ring). `friends` uses a literal blue hex (no blue theme token; documented as an intentional exception in-file). Labels/structure/prop-unions = plan verbatim; unions byte-identical to T12 `RecCard.confidence`/`.slot`; no emojis; no `"use client"`. 6/6 acceptance, 108/108 recs suite no regression.
> - **Carry-forward to Task 15** (code-review recommendations, deferred from T14 as out-of-scope-for-an-atom): (a) when RecCard wires `<ConfidencePill>`/`<SlotBadge>`, pass the props narrowed from the `RecCard` field type (e.g. `rec.confidence`/`rec.slot` directly) so a future `recommendations.slot` enum widening surfaces as a compile error at the call site rather than a silent unstyled chip (the atoms locally redeclare the unions by design — decoupling UI from the `"use server"` module). (b) Add 1-2 styling-regression assertions in the T15 rec-card test (e.g. `expect(html).toContain("var(--accent)")` for a strong/owned card) since the atom tests assert text only — a token swap renders wrong but passes text-only tests.
...
> **Execution note (2026-05-15) — plan code was ~75% fictional + behavior-critical; shipped real-grounded in `86bc16d` + `9c0174e`.** The plan's `{rec}`-only signature / `await playRec(rec.id)` / shadcn-DropdownMenu+lucide / `bg-brand-purple` / RTL test would have regressed live UX. Spec bar = PRESERVE all real behavior + LAYER IN v2. **Preserved verbatim:** the 4-prop signature `{ rec, connectedPlatforms, gamePlatforms, onDismissed }` + local `Platform` type (T18 + the live `_client.tsx:281` caller depend on it), framer-motion `motion.div`+exit (no-visual-loss constraint), `doDismiss` (onDismissed→startTransition), `onSave`→saveRecForLater+sonner toast, the `PlayThisButton` sub-component + `onPlayWithPlatform`→`playRec` result routing (`router.push(/games/${slug})`, two toast.error branches), `art=posterUrl??coverUrl`, CSS-var tokens, plain-button/local-popover idiom (NO shadcn/lucide/emoji). **Added:** LibraryBadge (top-left↔inLibrary) + ConfidencePill (top-right↔slot!=="wildcard") replacing the removed decorative AI/basic badge+`isAi` (deliberate spec intent); full unclamped reasoning; chips row (SlotBadge + timeFit + per-mood + friends `/N played/`); Snooze/Never split (local popover; "Not for me"→snoozeRec, "Never show this again"→neverAgainRec, both via doDismiss; trigger reads "Not interested ▾" since "Not for me" is its first item — aria-label="Dismiss" + item labels are the pinned strings); `data-testid` rec-card / rec-card-dismiss / rec-card-snooze / rec-card-never / rec-card-play / rec-card-save (T20 hooks). JSDoc fully rewritten to v2. 7/7 acceptance, tsc clean, full suite 876/876 no regression. Test = renderToStaticMarkup (no RTL/DOM dep) — popover-open interaction deferred to T20 Playwright (the honest no-DOM boundary).
> - **Parking-lot carry-forwards (code-review Minors, none blocking — deferred as out-of-scope refactors of correct/tested behavior-critical code):** (M3) extract a `DismissSplit` sub-component mirroring `PlayThisButton` (removes the lone parent/child `openDismiss` state-coupling + makes the file self-consistent) — best structural cleanup, do post-rollout. (M5/M6) both rec popovers (`PlayThisButton` picker + the new dismiss split) lack outside-click/Escape close + ARIA menu semantics/focus-trap — **pre-existing app-consistent limitation, NOT introduced here**; if ever addressed, do BOTH popovers together (ideally via the M3 extraction sharing a `useOutsideClick`). (M1) `timeFitLabel` nested ternary → `Record` map for consistency with the T14 atoms. (M7) add 2 static tests for `PlayThisButton` 1-overlap/2+-overlap variants (byte-preserved, no prior unit test; T20 covers interaction).
...
> **Execution note (2026-05-15) — plan scaffolding was fictional-for-this-repo; shipped real-grounded in `1eea2a6`.** Verified: `@/components/ui/popover` does NOT exist; `@/lib/games/types` has no `Platform` export (rec-card declares it locally); `@testing-library/react` not a dep / vitest `environment:"node"`. Authorized substitutions: local-state popover idiom mirroring rec-card's `PlayThisButton` (NO shadcn Popover/Button); NO `lucide-react` (text `▾` caret; selected = `font-medium text-[var(--accent)]` style, no icon); CSS-var tokens matching rec-card's exact vocabulary (NOT `bg-muted`); local `type Platform = "steam"|"xbox"|"psn"` byte-identical to rec-card; `Mood`/`TimeBudget` `import type` from `@/lib/recs/moods` (option `value`s match the unions). Plan's discriminated-union LOGIC preserved exactly (3 variants; `renderChipLabel`; time select+auto-close; mood multi toggle, max-2 via `disabled=!selected&&value.length>=2`, no auto-close; platform multi from `props.options`, no cap, no auto-close). Added `data-testid` `filter-chip-{variant}` (trigger) + `filter-chip-option-{value}`. The discriminated union is verified (project-wide tsc clean) to give T18 the correctly-narrowed per-variant `onChange` signature. Trigger has `aria-haspopup`/`aria-expanded` (a *positive* a11y divergence over PlayThisButton — keep). Test = renderToStaticMarkup label-derivation (8 tests); interaction (open/select/onChange/close/mood-cap) deferred to T20 (no-DOM boundary). 884/884 no regression.
> - **Carry-forwards:** (T18) consume a shared `Platform` type rather than hand-declaring a 3rd copy — recommend adding `export type Platform = "steam"|"xbox"|"psn"` to the plain (non-`"use server"`) `lib/recs/moods.ts` (it already cleanly exports `Mood`/`TimeBudget`) and refactor rec-card + filter-chip to import it. (T20) the mood max-2 `disabled` computation + the toggle add/remove array logic are the only non-trivial interaction logic in this file and are statically unverifiable — T20 Playwright MUST explicitly assert the 3rd-mood-disabled state AND the deselect path. (Parking lot) this popover's lack of outside-click/Escape + ARIA-menu semantics folds into the SAME existing T15 M5/M6 item (address all recs popovers together; when done, bring `aria-haspopup`/`aria-expanded` back to `PlayThisButton`).
...
> **Execution note (2026-05-15) — plan scaffolding fictional-for-this-repo; shipped real-grounded in `9aeba77` + `df07cbf`.** Verified: `@/components/mascot/illustration`/`MascotIllustration` does NOT exist; "Cortez" appears NOWHERE in product code (only docs/dev-account) — copy is name-free. Real-grounded: composes the EXISTING `<MascotPrompt mood="pointing">` wrapper (`@/components/recs/mascot-prompt`, which renders `<Mascot size="md" silent />` + a bordered `var(--bg)` box); copy `"Not landing? Tell us what to try:"`, placeholder `'e.g. "less grindy"'`; plain `<input>`/`<button>` + CSS-var tokens matching rec-card/filter-chip; NO shadcn ui/* , NO lucide (text `×`/`→`), no emoji. The bug-prone core extracted as a pure exported `applyRefinement(active, raw)` (collapse `\n`/trim/slice-140/dedupe-&-empty→SAME-ref/append-then-cap-5-drop-oldest-immutably) — 5 direct unit tests cover criteria 6/7 (NOT deferred). renderToStaticMarkup static tests (no vi.mock needed — Mascot SSRs clean); DOM interaction (chip-click/Enter/remove/clear) deferred to T20. Added `aria-label="Refine recommendations"` on the input (a11y — it's the only input in components/recs/, sets the family convention) + a JSDoc onChange contract. data-testids: refinement-input/submit/quick-{chip}/remove-{r}/clear-all. 9/9 acceptance, full suite 893/893 no regression.
> - **Carry-forwards to Task 18** (code-review): (1) `onChange` emits the FULL set every user mutation; it is skipped ONLY on a no-op `commit` — `remove`/`clearAll` always emit a fresh array, so T18 MUST dedupe `refinements` BY VALUE (not array identity) before triggering a `getRecs` refetch, or it will refetch-loop. (2) `MascotPrompt` hard-codes `mb-6` bottom margin — T18 layout must account for it (RefinementInput can't be placed flush). (3) From T16: prefer adding a shared `export type Platform = "steam"|"xbox"|"psn"` to `lib/recs/moods.ts` and have T18/rec-card/filter-chip import it rather than a 3rd hand-declaration. (4) From T15: `_client.tsx:281` is the current `<RecCard>` callsite (4-prop contract `{rec,connectedPlatforms,gamePlatforms,onDismissed}` + framer-motion AnimatePresence optimistic-dismiss) — T18 must preserve that wiring.
...
> **Execution note (2026-05-15) — plan ~60% fictional; shipped real-grounded in `0980e51` + `1ad8287`.** `page.tsx` was NOT rewritten (the plan's `loadUserConnectedPlatforms`/`@/lib/platforms/server-helpers`/server-side-getRecs/`Suspense`+`PlayNextSkeleton` were fictional — real page.tsx already loads via `getFingerprint`+`listConnections`, `loading.tsx` is auto). Only `_client.tsx` changed. Props UNCHANGED `{initialParams,tier,userConnectedPlatforms}`. PRESERVED byte-faithful: the RecCard 4-prop call (`rec`/`connectedPlatforms`/`gamePlatforms`-predicate/`onDismissed`) inside `<AnimatePresence mode="popLayout">`, empty-tier short-circuit, `dismissedIds`/`onCardDismissed`, `loadRecs`/`refillRecs`, RecResult ok/banner/!ok-reason states, `pendingCopyFor`, CSS-var tokens, framer-motion. REMOVED the wizard (editingStep machine + step screens + `FilterChips`/`FilterPills`/`Pill`). ADDED always-visible `FilterChipPopover×3` + `RefinementInput`, refine-URL-state, default filters (time="1hr"/moods=["chill"]/platforms=connected), value-deduped refetch. Code-review found + fixed an Important bug: the `refine` URL round-trip double-encoded on write + never decoded on read (`"shorter, cozier"` split into two on reload) — fixed to repeated `refine` query keys (URLSearchParams owns encoding; array-branch read; moods/platforms stay CSV — enum-safe); also `join("\n")` collision-proof dedupe, same-time-reclick guard (no spurious rerank), `recsState===null`→loading affordance, shared `MAX_ACTIVE`. tsc clean, 893/893 no regression, all 9 acceptance criteria; interaction/visual → T20 Playwright.
> - **Carry-forwards:** (parking-lot M3) refactor `time` state `TimeBudget|""` → non-nullable `TimeBudget` (seed `initTime||"1hr"`) and drop the `*OrDefault` indirection — a type-clarity cleanup deferred (broader refactor of spec-passed init-parsing; not done mid-task). (Known pre-existing, NOT a T18 regression) `refillRecs(rawFilters)` is filters-only by the existing server contract — "Show me more like these →" does NOT re-apply an active refinement (documented in a code comment); if desired later, that needs a server-action signature change, out of scope. (T20) assert: deep-link `?refine=` with a comma round-trips as ONE refinement; same-time chip re-click does NOT fire a 2nd rerank; initial paint shows the thinking-mascot not a blank grid.
...
> **Execution note (2026-05-15) — plan code had 3 repo-fatal bugs; shipped real-grounded in `84fb9a1` + close-out hardening.** Three controller-authorized corrections of fictional plan code, all verified by the spec reviewer as required (not gaps): (1) plan's `env.NODE_ENV !== "production"` — the parsed `@/lib/env` object has NO `NODE_ENV` key (Next injects it per-env, intentionally not in the zod schema); shipped reads `process.env.NODE_ENV` directly. (2) plan's schema `RECS_V2_ENABLED: z.string().optional().transform((s)=>s==="true")` collapses *unset* and `"false"` into the same `false` boolean, destroying the load-bearing tri-state (unset → dev/prod default; explicit `"true"`/`"false"` → override); shipped keeps it `optionalString` and does the tri-state string compare in `feature-flag.ts`. (3) plan only added keys to the `serverSchema` z.object — but `lib/env.ts` ALSO has an explicit key-by-key `serverSchema.parse({ …process.env.X… })` mapping; keys absent there are never read at runtime, so both `RECS_V2_ENABLED`/`RECS_V2_USERS` were added to BOTH the schema AND the explicit parse mapping. **Rollout decision (operator-confirmed earlier for T12):** no preserved legacy client — the server action delegates to the byte-preserved `getRecsLegacy` when off (import + ONE branch line `if (!isRecsV2Enabled(me.id)) return getRecsLegacy(rawFilters);` after the `!me` guard; v2 body + `getRecsLegacy` byte-untouched), and non-canary users get a static "being upgraded" page (CSS-var tokens, not the plan's fictional `text-muted-foreground`/`container`). Resolution order: canary list (`RECS_V2_USERS`, comma-split+trimmed) → `RECS_V2_ENABLED==="true"` → `==="false"` → `process.env.NODE_ENV!=="production"`; `import "server-only"` (no client path can force either branch; `userId` is server-trusted via `getCachedUser`). **Post-review hardening (both reviews passed; code-quality APPROVED with only Minor/optional):** applied note #1 — `env.RECS_V2_ENABLED?.trim().toLowerCase()` before the tri-state compare so an operator flipping the *emergency rollback switch* under pressure (`" FALSE "`, `"True"`) still gets the intended state (spec values `true`/`false` behave identically; `"1"`/`"0"` still fall to default, documented); added a 5th feature-flag test pinning the normalization; applied note #2 (doc comment now explains *why* NODE_ENV is read direct, to inoculate a future "fix"). **Carry-forward (deferred, optional refactor of correct code):** parse `RECS_V2_USERS` to a `string[]` at the env-schema layer (mirroring sibling `ADMIN_USER_IDS` at `env.ts:52-55` — once-at-parse vs per-call split, idiom-consistency); no downstream task depends on it. tsc clean; `feature-flag.test.ts` 5/5; full recs unit suite 137/137; T12 integration `tests/integration/recs/get-recs.test.ts` + `dismissal-actions.test.ts` 13/13 no regression (flag defaults true in test env so the v2 path is still exercised). 5/5 acceptance criteria.
...
> **Execution note (2026-05-15) — plan test code 100% fictional-for-this-repo; shipped real-grounded in `605c309`.** The plan's magic-link login / `@playwright/test` direct import / `tests/e2e/_helpers/auth.ts` + `tests/e2e/_fixtures/seed-recs-v2.ts` / `getByRole(/1 hour/)`/`getByText("15 min")`/`time=15min`/`refine=less-grindy`/`getByLabel('Remove …')` selectors / "≥50 logs+≥10 library+≥2 dismissed" seeding were ALL fictional. Real-grounded against verified contracts: imports `../fixtures/test-base`; password-form `loginAs` mirroring `phase6-pageant.spec.ts` (`#email`/`#password` ids — dodges the magic-link toggle's `aria-label` collision); `seedLogs` capped at `count:13` (the `SEED_GAME_IDS` pool — unique idx on (user_id,game_id,is_replay) makes >13 impossible); **no library/dismissed seed helpers exist, so the plan's seeding criteria were correctly dropped — unnecessary for the achievable assertions** (YAGNI, no new helpers). Real selectors used: `[data-testid=filter-chip-{time,mood,platform}]`/`filter-chip-option-{value}`, `refinement-input`/`refinement-submit`/`refinement-remove-<text>`, `rec-card-dismiss`(aria "Dismiss")/`rec-card-snooze`("Not for me")/`rec-card-never`("Never show this again"); URL is `?time=<v>&moods=<csv>` + REPEATED `refine` keys (space→`+`), defaults time `1hr`/moods `["chill"]`. **Only source change: 1 line `data-slot={rec.slot}` on the rec-card root `<motion.div>`** (T15 shipped only `data-testid="rec-card"`; the plan's Step 2 was still owed) — verified it does NOT perturb `rec-card-v2.test.tsx` (7/7 green; those assert SlotBadge *text*, not the attribute). **Three carry-forward regressions folded in + spec-review-verified faithful:** (a) mood max-2 cap — default `["chill"]`(1)→select 2nd→3rd option `toBeDisabled` (matches `filter-chip-popover.tsx:131` `!selected && value.length>=2`), deselect→`toBeEnabled` (mood popover correctly does NOT auto-close); (b) deep-link `?refine=cozy, short` (ONE key, literal comma) round-trips as exactly ONE pill (`toHaveCount(1)` + visible `refinement-remove-cozy, short`) — proves the comma is NOT split, guarding `_client.tsx:54-63` (`Do NOT parseCsv here`); (c) initial-paint thinking-mascot — loose regex over all 5 real `pendingCopyFor()` variants, asserting the *stable* `recsState===null` pending state (not a transient), positive signal = first rec-card visible. **Sanctioned tolerances** (because v2 `getRecs` degrades gracefully — rerank-fetch failure → `metadataOnlyRecs`+banner, so card count is non-deterministic locally): asserts `>=1` rec-card (NOT a hard 6), conditional wildcard, banner-agnostic. tsc clean. Spec-compliance review: **SPEC COMPLIANT** (6/6 items, 3/3 carry-forwards faithful). Code-quality review: **APPROVED** — zero Critical/Important; the 3 Minor/optional notes are explicitly no-action (duplicated `loginAs` = standard cross-spec Playwright tradeoff; `|%20` alternation = deliberate defensive dead-but-harmless; per-test seeding = correct fixture isolation since T1 also seeds `publicUser2`+follow).
>
> **⚠ OPERATOR CLOSE-OUT PREREQUISITE (blocks T21 runtime + any live /play-next) — carry-forward:** migration `lib/db/migrations/0018_play_next_redesign.sql` (the `rec_slot` enum + `recommendations.slot`/`dismissed_at`/`snoozed_until`/`never_again` + partial `recommendations_neg_lookup_idx`) is **NOT applied to the dev Supabase** — verified absent via Supabase MCP (no type, no columns, no index; 15 existing `recommendations` rows will take the safe backfilling defaults `slot='comfort'`/`never_again=false`, exactly as the migration's own comment anticipates → zero lock impact, purely additive). v2 `getRecs` SELECTs `slot` in its first cache-check query *before* the rerank `try/catch`, so `/play-next` 500s (global error boundary) until 0018 is applied — the implementer correctly refused to fake a passing run against the schema-broken backend. A controller attempt to apply it via Supabase MCP `apply_migration` was **correctly denied by the safety classifier** (live/shared-resource production DB change; the user's "Go ahead and start" authorized the coding plan, and this project's documented convention defers migration application + Edge Function deploy to an explicit operator close-out — see prior phases in memory). **Operator action required before T21's Playwright gate and before exercising live /play-next:** apply `0018_play_next_redesign.sql` to the dev Supabase (Supabase MCP `apply_migration`, name `play_next_redesign`) AND deploy the T11 `rerank-recs` rerank-only-mode Edge Function (deferred at T11). Until then T20's spec is statically correct + tsc-clean but cannot be executed locally; this is an environmental gate, not a T20 code defect.
...
> **Execution note (2026-05-15) — plan's ~40-line `execSync`/emoji sketch was fictional-for-this-repo; shipped real-grounded in `4372ccd`, tag `play-next-v2-complete` (local, NOT pushed).** Built to the real verify-script house style (mirrors `scripts/verify-phase-6.ts`/`-5.ts`): `Check[]`+`record()` with ASCII `PASS`/`FAIL`/`SKIP` (no emoji), `runGroup()` auto-FAIL-on-throw, `databaseUrl()`, gate-mapping + manual-gate summary, `process.exit(fail===0?0:1)` (SKIP never fails the gate), top-level `.catch`→exit(2). **5 groups:** G1 = 18 unit suites (17 named + whole-dir sweep), G2 = `get-recs` integration, **G3 = `pnpm build` — the canonical Next-16 `"use server"` export-validation gate the plan OMITTED (controller-added; this is the single most important gate — tsc/vitest/lint don't catch Server-Action export regressions, only `next build` does; a prior overhaul's prod deploy broke on exactly this)**, G4 = 7 source-wiring regex assertions (migration 0018, feature-flag resolution order, getRecsLegacy short-circuit, rec-card data-attrs, env.ts schema+parse ×2, T11 prompt-mirror, tasks.json 21/21), G5 = E2E (DB-introspects `rec_slot`/`recommendations.slot`; ABSENT → single SKIP with operator-close-out reason, does NOT run/hang Playwright; present → runs it). **Two justified deviations:** (1) G4.6 mirror asserts rerank-REGION byte-equality (anchor-bounded, the real invariant `tests/unit/recs/rerank-prompt.test.ts` enforces) + `RERANK_PROMPT_VERSION="v2"` in both files — NOT whole-file identity, which is impossible by necessity (`import "server-only"`+`@/` aliases vs Deno inline types, ~20 lines differ); (2) tasks.json Task 21 `pending`→`completed` (the gate run IS T21; G4.7 would self-FAIL otherwise — single-line, the only non-script change). **Actual run in this env (migration 0018 intentionally unapplied locally): `27/28 pass · 0 fail · 1 skip`, exit 0 — `pnpm build` genuinely PASSED**, all 18 unit suites + integration + 7 source assertions PASSED, E2E SKIP (operator-gated). tsc clean. Spec review: **SPEC COMPLIANT** (independently re-ran the script, both deviations confirmed justified). Code-quality review: **APPROVED** — reviewer empirically tested all three `spawnSync` failure modes (binary-not-found / non-zero / genuine-timeout all → FAIL, PASS strictly keyed on `status===0`): **no false-pass path exists** (critical for a merge gate); only Minor/optional no-action notes (the `shell:true`+DEP0190 is the correct Windows trade-off with static metacharacter-free args; `DATABASE_URL`-absent→exit-2 is a deliberate fail-loud matching verify-phase-6; G5 SKIP-on-ambiguity is the safe bias). **Operator close-out before production (carry-forward, restated for visibility):** apply `0018_play_next_redesign.sql` to dev+prod Supabase (Supabase MCP `apply_migration`, name `play_next_redesign`); deploy the T11 `rerank-recs` rerank-only-mode Edge Function; then the 4 manual checks (M1 visual 3×2 grid, M2 refinement <2s, M3 wildcard sanity ×3 users, M4 provider-failover smoke); then production rollout via the sequence below. Parking-lot refactors (non-blocking, recorded across T13/T15/T18 notes): shared `Platform` type, `saveRecForLater`/`playRec` soft-neg parity, popover outside-click/ARIA (M5/M6), `_client` M3 state-type, `refillRecs` filters-only, `RECS_V2_USERS` parse-at-schema (mirror `ADMIN_USER_IDS`).

> **Final whole-branch review (2026-05-15) — one Critical caught + fixed in `c8ff475`; branch now READY TO MERGE.** After all 21 per-task two-stage reviews passed, the mandatory whole-implementation review found a **Critical the isolated per-task reviews structurally could not catch**: `getRecsLegacy` (the spec-principle-8 one-toggle kill-switch / `recsv2`-OFF rollback path) projected `recommendations.slot` in its two `db.select`s — a coupling added incidentally in T12 only to satisfy the SHARED `hydrateRecs` row type. Because migration 0018 is a deferred operator step (NOT applied in prod), the OFF path would also throw Postgres `42703 undefined_column` → 500 for every real user *until* 0018 lands — i.e. "flip the flag off to recover" was a false safety net precisely in the pre-migration window where rollback matters most. The genuine pre-v2 code never selected `slot`. **Fix (the reviewer's preferred code option — restores true prior behavior, DRYer than a second row type, zero v2 change):** `RecHydrationRow.slot` made optional; `hydrateRecs` defaults it `?? "comfort"` (uniform with `metadataOnlyRecs`'s hardcoded `"comfort"`); the two legacy `slot: recommendations.slot` projections deleted; `getRecsLegacy` JSDoc rewritten to assert schema-independence; **new gate check `G4.8` added to `scripts/verify-play-next-v2.ts`** locking the invariant (slices the `getRecsLegacy` body only, FAILs if it ever reprojects `slot`, so a future edit can't silently re-couple). Controller independently verified the `getRecsLegacy` body (lines 117-386) now references NONE of the 0018 columns (`slot`/`dismissed_at`/`snoozed_until`/`never_again`) — the `where` uses the original pre-0018 `dismissed` boolean. Verification post-fix: tsc clean; recs unit+integration `18 files / 141 tests` green (v2 path is what runs in test env — proves no v2 regression); verify gate `28/29 pass · 0 fail · 1 skip`, exit 0, G4.8 PASS, G3 `pnpm build` PASS (only the expected operator-gated G5 Playwright SKIP). **Lesson:** a "preserved legacy/rollback path" must be verified schema-independent of the *same* change's deferred migration; per-task isolated review cannot see this — the whole-branch review is load-bearing, not ceremonial.
