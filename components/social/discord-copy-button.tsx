...
/**
 * Small client island for the Discord pill in <ConnectorPills>. Clicking
 * copies the handle to clipboard with a toast notification — the only
 * piece of <ConnectorPills> that needs to ship JS. Steam + Xbox pills
 * are plain anchors and render server-side.
 *
 * Discord profile URLs require shared-server membership and aren't
 * publicly resolvable, so there's no useful link target — copy is the
 * UX, mirroring Steam/Discord profile-share conventions.
 */
export function DiscordCopyButton({ handle }: { handle: string }) {
  return (
    <button
      type="button"
      onClick={() => {
        navigator.clipboard
          .writeText(handle)
          .then(() => toast.success(`Copied ${handle}`))
